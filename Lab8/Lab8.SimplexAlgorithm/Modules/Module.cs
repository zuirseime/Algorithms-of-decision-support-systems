...
public class Module {
    protected int FindPivotColumn(Tableau tableau, int row) {
        if (tableau.Data is null)
            throw new ArgumentNullException("Could not find a pivot column because the tableau is empty.");
    ...
    protected int FindPivotRow(Tableau tableau, int col) {
        if (tableau.Data is null)
            throw new ArgumentNullException("Could not find a pivot row because the tableau is empty.");
